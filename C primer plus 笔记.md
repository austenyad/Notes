...
C 不允许把数组作为 一个单元赋给另一个数组，除初始化以外也不允许使用花括号列表的形式赋 值。
...
int main(void){
  ...
  int yaks[SIZE];
  yaks = oxen; // 把一个数组赋值给另一个数组，不允许
  
  yaks[SIZE] = {4,3,2}; // 在数组定义好后，在使用 花括号 初始化数组 不允许
  
  yaks[SIZE] = oxen[SIZE]; // 这个等价于 yaks[5] = oxen[5]，数组的下标是[SIZE-1]，数组越界，不运行
	return 0;
}
```

C 语言 数组，要保证数组下标有效，编译器不会检查数组下标是否使用得当。在C标准中，使用越界下标的 结果是未定义的。这意味着程序看上去可以运行，但是运行结果很奇怪，或 异常中止。

C 语言为何会允许这种麻烦事发生？这要归功于 **C 信任程序员的原则**。不检查边界，C 程序可以运行更快。
...
而C99标准允许这样声明，这创建了一种新型数组，称为**变长数组 （variable-length array）**或简称 VLA（C11 放弃了这一创新的举措，把VLA 设定为可选，而不是语言必备的特性）。

<font size=4, color='red'><b> 上面说的标准在 Mac 电脑上 好像并没有遵循？</b></font>
